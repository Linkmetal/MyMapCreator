...
function createMap(){
    ...
    str = "<div class='toolbar'><button class='undo' onclick='undo()'>←</button><button class='redo' onclick='redo()'>→</button><span>Layer Selector:&nbsp</span><select class='layerSelector' id='layerSelector_" + name + "'><option value='0'>0</option><option value='1'>1</option><option value='2'>2</option><option value='3'>3</option></div>"
    $("#" + name).append(str);

    //CREACION DEL BOTON UNDO Y redo
    str = "";
    str +=
    $("#" + name).append(str);

    //ASIGNACION DEL TAMAÑO PARA LAS CELDAS
    ...
    $(".cell").click(function (event){
        ...
        if(($(cell).find("img").css("z-index") == zindex) == false){
            ...
            //PUSH EN LA PILA DE undo
            undo.push(new Array(cell.id,zindex));
            //APPEND DE LA IMAGEN
    		cell.append(img);
        }
        ...
        $(".cell > img").click(function(event){
            ...
            if(($(cell).find("img").css("z-index") == zindex) == false){
                ...
                img.style.top = "-" + top.toString() +"px";
                //PUSH EN LA PILA DE undo
                undo.push(new Array(name,zindex));
                //APPEND DE LA IMAGEN
        		cell.append(img);
            }
        });
